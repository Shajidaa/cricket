                            ...
                            <li key={child.name}>
                              <Link href={child.href}  passHref>
                                <NavigationMenuLink className={cn(
                                  "block select-none space-y-1 rounded-md p-3 leading-none no-underline outline-none transition-colors hover:bg-red-600 hover:text-white text-white font-medium",
                                  pathname === child.href && "text-red-600"
                                )}>
                                  {child.name}
                                </NavigationMenuLink>
                              </Link>
                  ...
                  <div key={item.name} className="flex flex-col">
                    {item.children ? (
                      <div className="flex flex-col gap-2">
                        <span className="text-xl font-bold text-white/50">{item.name}</span>
                        <div className="pl-4 flex flex-col gap-3 border-l border-white/20 ml-1">
                          {item.children.map((child) => (
                            <Link 
                              key={child.name} 
                              href={child.href}
                              className={cn(
                                ...
                                pathname === child.href ? "text-red-600" : "text-white"
                              )}
                            >
                              {child.name}
                            </Link>
